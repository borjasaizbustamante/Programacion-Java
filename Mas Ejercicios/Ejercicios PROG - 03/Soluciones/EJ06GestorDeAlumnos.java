 ...
 */
public class GestorDeAlumnosEj5 {
	...
	/**
	 * Recorre el array de alumnos y obtiene la media.
	 */
	private static void notaMediaClase() {
		int sumatorio = 0;
		int numAlumnos = 0;
		...
		for (int i = 0; i < arrayAlumnos.length; i++) {
			...
			if (null != arrayAlumnos[i]) {
				...
				String notaAlumno = getNotaDelAlumno(alumno);
				sumatorio = sumatorio + Integer.parseInt(notaAlumno);
				numAlumnos++;
			}
		}
	...
	/**
	 * Recorre el array de alumnos en busca de la nota mas alta y más baja. Las
	 * muestra por pantalla
	 */
	private static void notaMaximaYMinimaClase() {
		int notaMasBaja = 0;
		int notaMasAlta = 0;
		...
		for (int i = 0; i < arrayAlumnos.length; i++) {
			...
			if (null != arrayAlumnos[i]) {
				...
				if (notaMasBaja > notaAlumnoInt) {
					notaMasBaja = notaAlumnoInt;
				}

				if (notaMasAlta < notaAlumnoInt) {
					notaMasAlta = notaAlumnoInt;
				}
			}
		}
